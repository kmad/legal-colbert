#!/usr/bin/env bash
# Create an L40S pod, copy P5 inputs, and launch the tiny V9->LEDGAR run.
set -euo pipefail
...
OUT=$(prime pods create --id "$ID" --gpu-type L40S_48GB --gpu-count 1 \
  --image ubuntu_22_cuda_12 --name legal-colbert-p5 --disk-size 80 --yes --plain 2>&1)
echo "$OUT"
...
rsync -az -e "ssh $SSHOPT" \
  train_v2.py eval_v2.py eval_clause.py benchmark.py run_p5_remote.sh \
  data_p3_ledgar_docside data_p2a_acord_cuad eval_clause_mleb eval_contractnli_blind \
  ubuntu@"$IP":/home/ubuntu/legal-colbert/
rsync -az -e "ssh $SSHOPT" \
  output/legal-colbert-v9-spans-capped \
  ubuntu@"$IP":/home/ubuntu/legal-colbert/output/

echo "Launching P5 in tmux..."
ssh $SSHOPT ubuntu@"$IP" 'cd ~/legal-colbert && (which tmux || sudo apt-get install -y tmux >/dev/null 2>&1); chmod +x run_p5_remote.sh && tmux new-session -d -s p5 "bash run_p5_remote.sh > run_p5.log 2>&1" && sleep 2 && tmux ls'
echo ""
echo "Launched. POD=$POD IP=$IP"
echo "Watch:  ssh $SSHOPT ubuntu@$IP 'tail -f ~/legal-colbert/run_p5.log'"
echo "Copy back: rsync -az -e \"ssh $SSHOPT\" ubuntu@$IP:/home/ubuntu/legal-colbert/output/ ./output/"
echo "Terminate: prime pods terminate $POD --yes"
